...
function k(n) {
  ...
  return e.length ? (e.unshift({ name: "Все поддерживаемые форматы", extensions: e.map((a) => a.extensions[0]) }), e) : P;
}
function f(n) {
...
async function b(n, e, a) {
  ...
  for (const s of r) {
    const o = Date.now();
    try {
      const c = D(await s.run()), p = Date.now() - o;
      if (c.length)
...
function v(n) {
  ...
  for (const i of n) {
    const r = (i.origin ?? "") + "|" + f(i);
    e.has(r) || (e.add(r), a.push(i));
  }
  return a;
}
const A = "ru.albatros.wdx/attachment:add:workspace";
function L(n) {
  if (!n) return;
  const e = n.model;
...
async function j(n, e, a, i) {
  ...
  for (let t = 0; t < a.length; t++) {
    ...
    try {
      i.appendLine(`… ${o}: подключаю (${t + 1} из ${a.length})`), await n.manager.eval(A, { workspace: s }), i.appendLine(`+ ${o}: подключён`), r.push({ name: o, status: "added" });
    } catch (c) {
      const p = F(c, o);
...
async function S(n, e) {
  e.clear(), e.show(), e.appendLine("ДИАГНОСТИКА НашеПО · Создание проекта"), e.appendLine(y), W(n, e), _(n, e), O(n, e), T(n, e), await I(n, e), e.appendLine(y), e.appendLine("Отчёт готов. Скопируйте его целиком.");
}
function W(n, e) {
  ...
  const i = a.workspace;
  e.appendLine(`   workspace.origin: ${i?.origin ?? "нет"}`), e.appendLine(`   workspace.root.title: ${i?.root?.title ?? "нет"}`), e.appendLine(`   workspace.inmemory: ${i?.inmemory}`), e.appendLine(`   модель проекта найдена: ${l(n) ? "да" : "нет"}`);
}
function _(n, e) {
...
async function I(n, e) {
  ...
  let a;
  try {
    a = await n.openDialog({ buttonLabel: "Проверить", filters: k(n) });
  } catch (i) {
...
async function w(n, e, a) {
  ...
  try {
    ...
    if (R(r)) {
      i.appendLine("Отменено пользователем.");
      return;
    }
...
function q(n) {
  ...
  return w(n, "Создание проекта", async () => {
    const a = v(await b(n, "Выберите файлы, которые нужно открыть в одном проекте", e));
    if (!a.length) return;
    e.appendLine(`Выбрано файлов: ${a.length}`);
    for (const s of a) e.appendLine(`  · ${f(s)}`);
    ...
    if (!r) {
      ...
      if (s.value === "quick") {
        const o = a[0];
        e.appendLine(`Быстрый проект на основе файла: ${f(o)}`);
        const c = await n.manager.openWorkspace(o);
        if (r = await $(c), t = a.slice(1), await u(n, c) || e.appendLine("Вид проекта не готов, подключение файлов может не сработать."), !r) {
          e.appendLine("Не удалось получить модель проекта из первого файла."), e.show(), await n.showMessage(`Файл «${f(o)}» не открылся как проект.`, "error");
          return;
        }
        e.appendLine("Первый файл лежит в теле проекта, а не в списке вложений. Это плата за скорость.");
      } else {
        e.appendLine("Создаю пустой проект штатным способом программы.");
        const o = await K(n);
        if (!o) {
...
function Q(n) {
  ...
  return w(n, "Добавление файлов", async () => {
    ...
    const i = v(await b(n, "Выберите файлы для добавления в проект", e));
    i.length && (e.appendLine(`Добавление файлов в проект: ${i.length}`), await E(n, e, await j(n, a, i, e)));
  });
}
async function E(n, e, a) {
  const i = H(a);
  e.appendLine(i);
    ...
    try {
      for (let o = 0; o < i.length; o++) {
        ...
        try {
          ...
          r++, e.appendLine(`- ${p}: ${g?.message ?? String(g)}`);
        }
      }
      t.percents = 100;
    } finally {
